...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['clear_rfid']) && $_POST['clear_rfid'] === 'true') {
            ...
            echo json_encode(['status' => 'cleared']);
        } elseif (isset($_POST['book_registration_scan']) && $_POST['book_registration_scan'] === 'true') {
            // During book registration, return the next barcode from the phone queue.
            ...
            if (file_exists($queueFile)) {
                ...
                if (!empty($lines)) {
                    $barcode = array_shift($lines);
                    file_put_contents($queueFile, implode("\n", $lines) . (count($lines) > 0 ? "\n" : ''));
                }
            }

            // Fallback: also check the hardware scanner file.
            if ($barcode === '') {
                $barcode = trim(file_get_contents(__DIR__ . '/last_barcode.txt'));
                if ($barcode !== '') {
                    file_put_contents(__DIR__ . '/last_barcode.txt', '');
                }
            }

            if ($barcode !== '') {
                $stmt = $pdo->prepare("SELECT id, title, book_number FROM books WHERE barcode = ?");
                $stmt->execute([$barcode]);
                $book = $stmt->fetch(PDO::FETCH_ASSOC);

                $remaining = 0;
                if (file_exists($queueFile)) {
                    $remaining = count(array_filter(file($queueFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES)));
                }
        ...
        } elseif (isset($_POST['registration_scan']) && $_POST['registration_scan'] === 'true') {
            // During student registration, return a raw RFID even when it is not registered yet.
            $rfid = trim(file_get_contents('last_rfid.txt'));
            if ($rfid !== '') {
                echo json_encode([
                    'status' => 'success',
                    'rfid_number' => $rfid
