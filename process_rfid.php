...
$borrow_time = isset($_POST['borrow_time']) ? trim($_POST['borrow_time']) : '';
...
// Validate borrow_time format (12-hour with AM/PM) and reasonableness
if (!preg_match('/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} (AM|PM)$/', $borrow_time) || strtotime($borrow_time) < strtotime('2020-01-01')) {
    error_log("Invalid borrow_time received: $borrow_time, using current PHT time");
    $borrow_time = date('Y-m-d h:i:s A'); // Current PHT in 12-hour format
}
...
try {
    ...
    if (strtoupper($action) == "BORROW") {
        ...
        // Insert transaction with borrow_time in 12-hour format
        ...
        // Insert return transaction with current time in 12-hour format
        $return_time = date('Y-m-d h:i:s A'); // Current PHT in 12-hour format
...
function notifyStudent($user_id, $email, $book_title, $action, $due_date, $timestamp) {
    ...
    if (!$notice_query->execute([$user_id, $message, $timestamp])) {
        error_log("Failed to insert notice: " . print_r($pdo->errorInfo(), true));
    }
